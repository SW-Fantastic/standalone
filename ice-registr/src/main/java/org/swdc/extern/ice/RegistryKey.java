 ...
 * @version 3.1.3
 *
 * @see org.swdc.extern.ice.Registry
 * @see org.swdc.extern.ice.RegistryValue
 */
...
public class RegistryKey implements Closeable {
	 ...
	 */
	private static class RegistryKeyEnumerator implements Enumeration<String> {
		RegistryKey		key;
		int				currIndex;
		int				numSubKeys;

		RegistryKeyEnumerator( RegistryKey key ) throws RegistryException {
			this.key = key;
			this.currIndex = 0;
	...
	private static class RegistryValueEnumerator implements Enumeration<String> {
		RegistryKey		key;
		int				currIndex;
		int				numValues;
	...
	export( PrintWriter out, boolean descend )
		...
		if ( descend )
			{
			iter = this.keyElements();

			for ( int idx = 0 ; iter.hasMoreElements() ; ++idx )
				{
				String keyName = (String) iter.nextElement();
